...
(defn- with-home
  ...
  [f]
  (let [home (str (fs/create-temp-dir {:prefix "restate-ssh"}))]
    (try
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/with-machine-key (assoc (keygen) :green/event :create))]
        (is (= (str (io/file home ".ssh" "restate-keygen-fixture")) (:ssh-private-key-path opts)))
        (is (= (str (io/file home ".ssh" "restate-keygen-fixture.pub")) (:ssh-public-key-path opts)))))))
...
(deftest identity-args-select-the-generated-key-only-in-keygen-mode
  ;; The acceptance step's reboot ssh threads these: in keygen mode nothing
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/ensure-key! (assoc (keygen) :green/event :create) (constantly nil))
            prv (io/file home ".ssh" "restate-keygen-fixture")
            pub (io/file home ".ssh" "restate-keygen-fixture.pub")]
        ...
        (testing "ed25519, no passphrase, profile-named comment"
          ...
          (is (str/includes? (slurp pub) "restate-keygen-fixture managed by Colors")))
...
(deftest a-key-without-state-is-never-overwritten
  (with-home
    (fn [home]
      (let [prv (str (io/file home ".ssh" "restate-keygen-fixture"))]
...
(deftest preflight-refuses-a-foreign-key-and-says-do-not-delete-it
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "restate-keygen-fixture.pub")) "ssh-ed25519 OURS comment")
      (with-redefs [once-ssh/fetch-account-keys
                    (fn [_ _] [{:id "abc" :name "restate-keygen-fixture" :public "ssh-ed25519 THEIRS"}])]
...
(deftest delete-removes-the-keypair
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "restate-keygen-fixture")) "private")
      (write! (str (io/file home ".ssh" "restate-keygen-fixture.pub")) "public")
      (ssh/cleanup-step (assoc (keygen) :green/event :delete :ssh-keygen true))
      (is (not (.exists (io/file home ".ssh" "restate-keygen-fixture"))))
      (is (not (.exists (io/file home ".ssh" "restate-keygen-fixture.pub"))))
...
(deftest cleanup-is-inert-on-create-and-in-opt-out-mode
  (with-home
    (fn [home]
      ...
      (is (.exists (io/file home ".ssh" "restate-keygen-fixture")))
