(ns io.github.getcolors.restate.ssh-config-test
...
(defn- with-config
  ...
  [content f]
  (let [home (str (fs/create-temp-dir {:prefix "restate-ssh-config"}))
...
(deftest alias-is-the-profile
  (is (= "restate-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/restate-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  ;; too would repeat it: "# BEGIN restate restate-digitalocean".
  (is (= "# BEGIN restate-digitalocean ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "restate-digitalocean")))
  (is (= "# END restate-digitalocean ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "restate-digitalocean"))))
...
(deftest owned-markers-hold-the-one-marker
  ...
  (is (= {:begin #{"# BEGIN restate-digitalocean ANSIBLE MANAGED BLOCK"}
          :end #{"# END restate-digitalocean ANSIBLE MANAGED BLOCK"}}
         (ssh-config/owned-markers "restate-digitalocean"))))

;; §5 never adopt

(deftest host-patterns-are-read-from-a-host-line
  (is (= ["restate-fixture"] (ssh-config/host-patterns "Host restate-fixture")))
  (is (= ["web" "restate-fixture" "db"] (ssh-config/host-patterns "  host   web restate-fixture  db ")))
  (is (nil? (ssh-config/host-patterns "    HostName 192.0.2.1")))
  (is (nil? (ssh-config/host-patterns "Match host restate-fixture"))))

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host restate-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "restate-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "restate-fixture"
...
(deftest a-block-under-a-package-prefixed-marker-is-foreign
  ...
  (let [alias "restate-digitalocean"
        lines [(str "# BEGIN restate " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END restate " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web restate-fixture db"]
                                           "restate-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host restate-other"]
                                            "restate-fixture"))))
  ...
  (with-config "Host other\n    HostName 192.0.2.1\n\nHost restate-fixture\n    User root\n"
    (fn [file]
      (let [err (ssh-config/adopt-error (fixture))]
        ...
        (is (str/includes? err "`Host restate-fixture` at line 4"))
        (is (str/includes? err "will not overwrite it"))))))

(deftest adopt-error-passes-our-own-block-and-a-missing-file
  (with-config (str (ssh-config/begin-marker "restate-fixture") "\n"
                    "Host restate-fixture\n    HostName 192.0.2.1\n"
                    (ssh-config/end-marker "restate-fixture") "\n")
...
(deftest preflight-reads-the-redirected-file
  ...
  (with-config "Host restate-fixture\n    HostName 192.0.2.1\n"
    (fn [_]
      (let [r (ssh-config/preflight! (fixture))]
        (is (= 1 (:green/exit r)))
        (is (str/includes? (:green/err r) "already declares")))))
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/restate-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "restate-ansible-local") targets))))
...
(deftest the-rendered-play-carries-the-identity-pair-only-in-keygen-mode
  (let [keygen-play (render-play (keygen))
    ...
    (is (str/includes? keygen-play "IdentityFile ~/.ssh/restate-keygen-fixture"))
...
(deftest create-writes-the-block-after-compute-and-before-convergence
  (is (= [:restate/ssh-config]
         (vec (rest (workflow/wire-fn :restate/infrastructure {:green/event :create})))))
  (is (= [:restate/dns]
         (vec (rest (workflow/wire-fn :restate/ssh-config {:green/event :create}))))))
...
(deftest delete-removes-the-block-before-the-destroy
  ...
  (is (= [:restate/ssh-config]
         (vec (rest (workflow/wire-fn :restate/dns {:green/event :delete})))))
  (is (= [:restate/infrastructure]
         (vec (rest (workflow/wire-fn :restate/ssh-config {:green/event :delete})))))
  (is (= [:restate/ssh-cleanup]
         (vec (rest (workflow/wire-fn :restate/infrastructure {:green/event :delete}))))))
