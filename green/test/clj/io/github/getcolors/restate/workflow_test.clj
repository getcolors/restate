...
(deftest real-delete-requires-credentials
...
(deftest a-real-create-on-a-fresh-work-directory-reports-the-credentials-not-a-crash
  ...
  (let [work (str (fs/create-temp-dir {:prefix "restate-fresh"}))]
    (try
...
(deftest delete-fails-loudly-when-state-is-unreadable
  ;; This package used to swallow every exception in `state-output` and merge
  ;; `(or nil {})`: a stale backend credential would have pointed the cleanup
...
(deftest delete-has-no-address-override
  ;; ONCE's adoption applies no `:ip` override and this package adds none: the
  ;; recorded address wins over anything already in opts, so nothing can point
  ;; a delete's cleanup at an arbitrary host.
  (let [r (start (deletable-fixture :green/event :delete :ip "203.0.113.7")
                 {:provider "digitalocean" :ip "198.51.100.1" :user "root"})]
    (is (= 0 (:green/exit r)))
    (is (= "198.51.100.1" (:ip r)))))
...
(deftest graph-orders-private-stack
  ...
  (is (= [:restate/ansible]
         (vec (rest (workflow/wire-fn :restate/dns {:green/event :create})))))
  (is (= [:restate/acceptance]
         (vec (rest (workflow/wire-fn :restate/ansible {:green/event :create})))))
  (is (= [:restate/ansible]
...
(deftest delete-removes-the-config-block-before-the-destroy
  ...
  (is (= [:restate/dns]
         (vec (rest (workflow/wire-fn :restate/ansible {:green/event :delete})))))
  (is (= [:restate/ssh-config]
         (vec (rest (workflow/wire-fn :restate/dns {:green/event :delete})))))
  (is (= [:restate/infrastructure]
         (vec (rest (workflow/wire-fn :restate/ssh-config {:green/event :delete})))))
  (is (some #{:restate/ssh-config} workflow/side-effecting) "a dry-run never writes ~/.ssh/config"))
...
(deftest delete-removes-the-key-after-the-compute-destroy
  ...
  (is (empty? (rest (workflow/wire-fn :restate/ssh-cleanup {:green/event :delete}))))
  (is (some #{:restate/ssh-cleanup} workflow/side-effecting) "a dry-run delete touches no key"))
