...
<% endif %>resource "digitalocean_droplet" "restate" {
  name     = "<{ compute-name }>"
...
output "params" {
  value = { provider = "digitalocean", ip = digitalocean_droplet.restate.ipv4_address, user = "root", sudoer = "root", name = "<{ compute-name }>"<% if ssh-keygen %>, ssh_key_id = digitalocean_ssh_key.machine.id<% endif %> }
}
