...
(def workflow
  (-> (wf/workflow {:start :restate/start :wire-fn wire-fn :next-fn (fn [_ successors opts] (if (or (:restate/already-destroyed opts) (wf/failed? opts)) [] (mapv #(vector % opts) successors)))})
