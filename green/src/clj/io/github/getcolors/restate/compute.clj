(ns io.github.getcolors.restate.compute
...
(defn requirements [opts]
  (let [ssh (request/source-cidrs opts "ssh-sources" "restate-ssh-sources") http (request/source-cidrs opts "http-sources" "restate-http-sources")]
    {:single_host true :legacy_state_keys [(str (:profile opts) "/restate-infrastructure.tfstate")]
...
(defn attach [opts result]
  (cond
    ...
    (= "destroyed" (:status result)) (assoc opts :green/exit 0 :restate/already-destroyed true)
