...
(defn stamp-green [s sha]
  (when (re-find #"\(def \^:private restate-sha (?:nil|\"[0-9a-f]{40}\")\)" s)
    (str/replace-first s #"\(def \^:private restate-sha (?:nil|\"[0-9a-f]{40}\")\)"
                       (str "(def ^:private restate-sha \"" sha "\")"))))

(defn stamp-red [s sha]
  (let [pinned (str "\"package-restate-red\": \"github:getcolors/restate#" sha "\",")]
    (cond (str/includes? s "\"package-restate-red\": null,")
          (str/replace-first s "\"package-restate-red\": null," pinned)
          (re-find #"\"package-restate-red\": \"github:getcolors/restate#[0-9a-f]{40}\"," s)
          (str/replace-first s #"\"package-restate-red\": \"github:getcolors/restate#[0-9a-f]{40}\"," pinned))))

(def blue-unpinned-meta "# dependencies = []\n# ///")
(defn blue-pinned-meta [sha]
  (str "# dependencies = [\"package-restate-blue\", \"blue\", \"package-once-blue\", \"colors-compute-blue\"]\n"
       "#\n"
       "# [tool.uv.sources]\n"
       ...
       "# package-restate-blue = { git = \"https://github.com/getcolors/restate.git\", rev = \"" sha "\", subdirectory = \"blue\" }\n"
...
(defn stamp-blue [s sha]
  ...
  (cond (str/includes? s blue-unpinned-meta)
        (-> s
            ...
            (str/replace-first #"(?s)# UNPINNED:.*?RESTATE_LIB_ROOT=/path/to/restate\n"
                               "# Stamped by `bb pin`. RESTATE_LIB_ROOT=/path/to/restate still overrides the\n# pin with a working tree.\n"))
        (re-find #"restate\.git\", rev = \"[0-9a-f]{40}\"" s)
        (str/replace-first s #"restate\.git\", rev = \"[0-9a-f]{40}\""
                           (str "restate.git\", rev = \"" sha "\""))))

(def sites
  [{:path "../skills/package-restate-green/green" :stamp stamp-green}
   {:path "../skills/package-restate-red/red" :stamp stamp-red}
   {:path "../skills/package-restate-blue/blue" :stamp stamp-blue}])

(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "restate working tree is dirty; commit before pinning")) (System/exit 2))
        (not (str/includes? (str remotes) "origin/")) (do (binding [*out* *err*] (println "restate HEAD is not pushed")) (System/exit 2))
