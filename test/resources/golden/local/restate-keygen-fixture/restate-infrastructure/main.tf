...
resource "digitalocean_ssh_key" "machine" {
  ...
  public_key = trimspace(file("/home/build-placeholder/.ssh/restate-keygen-fixture.pub"))
}
...
resource "digitalocean_firewall" "restate" {
  name        = "restate-keygen-fixture-firewall"
...
output "params" {
  value = { provider = "digitalocean", ip = digitalocean_droplet.restate.ipv4_address, user = "root", sudoer = "root", name = "restate-keygen-fixture", ssh_key_id = digitalocean_ssh_key.machine.id }
}
