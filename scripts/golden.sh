...
# because the SSH Keypair Standard has two modes and a package conforms only
# if both hold. `colors.yml` is opt-out mode: it supplies an explicit key id
# and a name equal to the profile and must render the historical shape, byte
# for byte, creating no key resource -- the shape of the `restate-digitalocean`
# deployment, so a change there is a plan against whatever its state holds.
...
for variant in colors keygen; do
  ...
  main="$actual/restate-infrastructure/main.tf"
  ...
  if grep -rEq '([0-9]{1,3}\.){3}[0-9]{1,3}' "$actual/restate-ansible-local"; then
