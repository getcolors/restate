...
# and parity means both keygen and opt-out hold in every colour. Both select
# the one advertised provider and the r2 state backend.
#
...
build_variant() {
  ...
  (cd "$root/green" && RESTATE_LIB_ROOT="$root" ./green build -f "$tmp/$variant-green.yml" >/dev/null)
  (cd "$root/red" && RESTATE_LIB_ROOT="$root/red" ./red build -f "$tmp/$variant-red.yml" >/dev/null)
  (cd "$root/blue" && uv run python -m package_restate_blue build -f "$tmp/$variant-blue.yml" >/dev/null)
