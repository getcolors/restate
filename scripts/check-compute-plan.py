...
assert len(nodes) == 1, 'Restate must render exactly one node'
...
assert not (root / 'restate-infrastructure').exists(), 'old compute stage must not be applied'
assert (root / 'restate-dns/backend.tf.json').is_file(), 'application DNS remains separate'
