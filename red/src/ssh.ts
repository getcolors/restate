...
// create matrix, the DigitalOcean REST preflight, and a cleanup that runs only
...
// from `$HOME` and does not commit rendered output; restate does commit goldens,
...
// through — ONCE's table, not a literal, so a second provider would need no
// second branch here.
export function withMachineKey(opts: Opts): Opts {
  ...
  const profile = String(opts.profile ?? "restate");
