...
from package_restate_blue import ssh_config, tools, workflow
...
def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "restate-fixture"
...
def test_identity_file_keeps_the_tilde(home):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/restate-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    assert ssh_config.begin_marker("restate-digitalocean") == "# BEGIN restate-digitalocean ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("restate-digitalocean") == "# END restate-digitalocean ANSIBLE MANAGED BLOCK"
...
def test_owned_markers_hold_the_one_marker():
    ...
    assert ssh_config.owned_markers("restate-digitalocean") == {
        "begin": {"# BEGIN restate-digitalocean ANSIBLE MANAGED BLOCK"},
...
def test_host_patterns_are_read_from_a_host_line():
    assert ssh_config.host_patterns("Host restate-fixture") == ["restate-fixture"]
    assert ssh_config.host_patterns("  host   web restate-fixture  db ") == ["web", "restate-fixture", "db"]
    assert ssh_config.host_patterns("    HostName 192.0.2.1") is None
    assert ssh_config.host_patterns("Match host restate-fixture") is None


def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host restate-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "restate-fixture") == 4


def test_our_own_block_is_not_foreign():
    alias = "restate-fixture"
...
def test_a_block_under_a_package_prefixed_marker_is_foreign():
    ...
    alias = "restate-digitalocean"
    lines = [f"# BEGIN restate {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END restate {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web restate-fixture db"], "restate-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host restate-other"],
                                          "restate-fixture") is None
...
def test_adopt_error_names_the_file_and_the_line(home):
    ...
    assert "`Host restate-fixture` at line 4" in error
...
def test_adopt_error_passes_our_own_block_and_a_missing_file(home):
    ...
    write_config(home, f"{ssh_config.begin_marker('restate-fixture')}\n"
                       "Host restate-fixture\n    HostName 192.0.2.1\n"
                       f"{ssh_config.end_marker('restate-fixture')}\n")
    assert ssh_config.adopt_error(fixture()) is None
...
def test_preflight_reads_the_redirected_file(home):
    ...
    write_config(home, "Host restate-fixture\n    HostName 192.0.2.1\n")
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/restate-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("restate-ansible-local" in t for t in targets)
