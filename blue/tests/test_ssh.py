...
from package_restate_blue import ssh
...
def test_real_events_render_the_real_path(home):
    ...
    assert opts["ssh-private-key-path"] == str(home / ".ssh" / "restate-keygen-fixture")
    assert opts["ssh-public-key-path"] == str(home / ".ssh" / "restate-keygen-fixture.pub")
...
async def test_first_create_generates_the_keypair(home):
    ...
    prv = home / ".ssh" / "restate-keygen-fixture"
    pub = home / ".ssh" / "restate-keygen-fixture.pub"
    ...
    assert "restate-keygen-fixture managed by Colors" in pub.read_text()
...
async def test_a_key_without_state_is_never_overwritten(home):
    ...
    write(home / ".ssh" / "restate-keygen-fixture.pub", "ssh-ed25519 AAAA test")
    opts = await ssh.ensure_key({**keygen(), "blue/event": "create"}, none_state)
...
def test_delete_removes_the_keypair(home):
    write(home / ".ssh" / "restate-keygen-fixture", "private")
    write(home / ".ssh" / "restate-keygen-fixture.pub", "public")
    ssh.cleanup_step({**keygen(), "blue/event": "delete", "ssh-keygen": True})
    assert not (home / ".ssh" / "restate-keygen-fixture").exists()
    assert not (home / ".ssh" / "restate-keygen-fixture.pub").exists()
...
def test_cleanup_is_inert_on_create_and_in_opt_out_mode(home):
    ...
    assert (home / ".ssh" / "restate-keygen-fixture").exists()
