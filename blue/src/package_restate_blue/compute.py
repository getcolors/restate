...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'restate-ssh-sources')
    http = source_cidrs(opts, 'http-sources', 'restate-http-sources')
    return {'single_host': True, 'legacy_state_keys': [opts['profile'] + '/restate-infrastructure.tfstate'],
...
def attach(opts, result):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'blue/exit': 0, 'restate/already-destroyed': True}
    params = node(result)
