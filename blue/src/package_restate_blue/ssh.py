...
matrix, the DigitalOcean REST preflight, and a cleanup that runs only after a successful destroy. Reusing it
...
``$HOME`` and does not commit rendered output; restate does commit goldens, so
...
def with_machine_key(opts: dict) -> dict:
    ...
    would need no second branch here."""
    ...
    if not build:
        return opts
    profile = opts.get("profile") or "restate"
